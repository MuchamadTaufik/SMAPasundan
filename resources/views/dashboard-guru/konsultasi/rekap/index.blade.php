...
@section('container')
   ...
   <div class="card shadow mb-4">
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
         <h6 class="m-0 font-weight-bold text-dark">Daftar Konsultasi {{ $biodata->user->name }}</h6>
      </div>
      <div class="card-body">
         <a href="{{ route('konsultasi.laporan', ['biodata_id' => $biodata->id, 'jenis_kegiatans_id' => 2]) }}" class="btn btn-primary float-right mb-4">Download Laporan</a>
            ...
            <table class="table table-bordered" id="dataTable2" width="100%" cellspacing="0">
                  ...
                  <tbody>
                     ...
                     @foreach ($kegiatan as $data)
                        @if ($data->jenis_kegiatans_id === 2 && $data->tanggal !== null)
                              <tr>
                                 <td>{{ $counter }}.</td>
                                 ...
                                 <td>
                                    ...
                                    <a href="{{ route('konsultasi.jadwal', $data->id) }}" class="badge bg-warning border-0"><i class="bi bi-pencil-square"></i></a>
                                    <form action="{{ route('konsultasi.delete', $data->id) }}" method="post" class="d-inline">
                                          @method('delete')
                                          @csrf
